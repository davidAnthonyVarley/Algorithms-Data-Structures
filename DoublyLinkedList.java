    ...
    public void delete(DLLNode node) 
    {
     ...
     if (node==tail) {
      ...
      if ( previousnode==null && nextnode==null) {
    	  node = new DLLNode(null, null, null);
    	  head=node;
    	  tail=node;   }
      */
      
      //if pos = last node in list, change tail
      //if pos = 0, cahnge head
     ...
     public void makeUnique()
    {
      ...
      for ( DLLNode currentnode = head; currentnode!=null; currentnode=currentnode.next) { //1
    	  ...
    	  for ( DLLNode j=currentnode; j!=null;) {  //2
    		  ...
    		  else {//if (currentnode.data.compareTo(j.data)!=0 || currentnode==j) { //3.2
    			  j = j.next;
    		  } //3.2
